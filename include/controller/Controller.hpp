...
concept HasGet = requires(const T a , const std::string &req, Args ...args) {
  { a.Get(req, args...) } -> std::convertible_to<HttpResponse>;
};

template <typename T, typename ...Args>
concept HasPost = requires(const T a, const std::string &req, Args ...args) {
  { a.Post(req, args...) } -> std::same_as<HttpResponse>;
};

template <typename T, typename ...Args>
concept HasDelete = requires(const T a, const std::string req, Args ...args) {
  { a.Delete(req, args...) } -> std::same_as<HttpResponse>;
};

template <typename T, typename ...Args>
concept HasPatch = requires(const T a, const std::string req, Args ...args) {
  { a.Patch(req, args...) } -> std::same_as<HttpResponse>;
};

template <typename T, typename ...Args>
concept HasPut = requires(const T a, const std::string req, Args ...args) {
